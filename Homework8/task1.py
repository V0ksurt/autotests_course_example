...
def treatment_sum(our_tuple):
    """
    Функция сложения координат двух элементов кортежа, содержащая в себе обработчики исключений
    :param our_tuple: Кортеж
    :return: Сумма элементов кортежа или строка обработки исключения
    """
    if len(our_tuple) > 2:
        raise Exception('Много данных')
    try:
        our_tuple = our_tuple[0] + our_tuple[1]

    except TypeError:
        our_tuple = ('Нельзя сложить эти данные')

    except IndexError:
        our_tuple = ('Недостаточно данных')
