...
class Segment:
    def __init__(self, one_cort, two_cort):
        self.x1, self.y1 = one_cort
        self.x2, self.y2 = two_cort

    def length(self):
        """
        Нахождение длины нашего отрезка, с округлением до 2 знаков после запятой. Решается с помощью теоремы Пифагора
