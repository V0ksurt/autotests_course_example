...
class PersonInfo:
    def __init__(self, name, age, *podrazdelenie: str):
        self.name = name
        self.age = age
    ...
    def short_name(self):
        """
        Получаем строку Фамилия И.
        """
        return self.name[self.name.index(' ') + 1:] + ' ' + self.name[:self.name.index(' ')][0] + '.'

    def path_deps(self):
        """
        Получаем путь "Головное подразделение --> ... --> Конечное подразделение"
        """
